...
class QXmobile(unittest.TestCase):
    ...
    def swipeRight(self,pel):
        start=pel.location
        print("控件起始位置：%s" % start)
        self.driver.swipe(start['x'],start['y'],start['x']+400,start['y'],2000)
    ...
    def drawer_menu(self):
        ...
        try:
            ...
            self.driver.tap([(253,461)],1)  #此菜单元素定位一直没响应，改为坐标值定位
            time.sleep(1)
            try:
        ...
        try:
            ...
            try:
                ...
                self.driver.tap([(590,905)],1)  #此菜单元素定位位置有偏移，无效点击，改为坐标值定位
                time.sleep(2)
